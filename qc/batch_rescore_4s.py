#!/usr/bin/env python3
"""Batch re-score 230 Phase 2 score-4 questions (missed 1 criterion)."""
...
def call_llm(prompt):
    payload = json.dumps({
        ...
        "messages": [
            {"role": "system", "content": "You are a TEAS exam quality auditor. Output ONLY a valid JSON array. No markdown fences."},
...
def build_prompt(questions):
    ...
    return f"""Score each TEAS question on 5 criteria (0 or 1 each):
...
def main():
    ...
    total = len(ids)
    print(f'Re-scoring {total} score-4 questions in batches of {BATCH_SIZE}')
    ...
    for bi, batch in enumerate(batches):
        ...
        print(f'  Batch {bi+1}/{len(batches)}: {batch_updated}/{len(batch)} scored | running: {passing} pass / {failing} fail')
        time.sleep(1)
    ...
    c.execute('SELECT COUNT(*) FROM qc_results WHERE phase2_done=1 AND phase2_score < 4')
    low = c.fetchone()[0]
    print(f'\nDONE: {updated}/{total} re-scored, {errors} batch errors')
